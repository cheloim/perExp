...
function renderMarkdown(text: string): React.ReactNode[] {
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    if (m.startsWith('**') && m.endsWith('**')) {
      segments.push(<strong key={segments.length} className="font-semibold text-[var(--text-primary)]">{m.slice(2, -2)}</strong>)
    } else if (m.startsWith('*') && m.endsWith('*')) {
      segments.push(<em key={segments.length} className="italic">{m.slice(1, -1)}</em>)
    }
...
function processLine(line: string, lineIdx: number): React.ReactNode {
  ...
  if (trimmed.startsWith('## ') || trimmed.startsWith('### ') || trimmed.startsWith('#### ')) {
    ...
    return (
      <p key={lineIdx} className={`font-semibold text-[var(--text-primary)] mb-2 ${level === 1 ? 'text-lg' : level === 2 ? 'text-base' : 'text-sm'} leading-relaxed`}>
        {text}
      </p>
    )
  }
