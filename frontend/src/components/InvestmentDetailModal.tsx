...
function EvolutionRow({ label, price, currentPrice, currency }: { label: string; price: number | null; currentPrice: number | null; currency: string }) {
  ...
  return (
    <div className="flex items-center justify-between py-1.5">
      ...
      <div className="flex items-center gap-2">
        <span className="text-xs text-[var(--text-primary)] font-medium">{fmt(price, currency)}</span>
...
export function InvestmentDetailModal({ isOpen, investment, allInvestments, onClose, onEdit }: InvestmentDetailModalProps) {
  ...
  const totalCurrentValue = investments.reduce((s, i) => s + (i.current_value ?? i.cost_basis ?? 0), 0)
  ...
  return (
    <DetailModal
      isOpen={isOpen}
      onClose={onClose}
      title={investment.ticker ? `${investment.ticker} · ${investment.name || '—'}` : investment.name || 'Inversión'}
      subtitle={subtitle}
    >
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
          <div>
            ...
            <dd className="text-[var(--text-primary)] font-medium">{fmtQty(totalQuantity)} unidades</dd>
          </div>
          <div>
          ...
          <div>
            ...
            <dd className="text-[var(--text-primary)] font-medium">{fmt(totalCostBasis, currency)}</dd>
          </div>
          <div>
            <dt className="text-[var(--text-tertiary)] text-xs uppercase">P&L</dt>
            <dd className={`font-medium ${totalPnl >= 0 ? 'text-success' : 'text-danger'}`}>
              {totalPnl >= 0 ? '+' : ''}{fmt(totalPnl, currency)}
              <span className="ml-1 text-xs">({totalPnlPct >= 0 ? '+' : ''}{totalPnlPct.toFixed(2)}%)</span>
            </dd>
          </div>
        </div>
          ...
          <div className="border-t border-[var(--border-color)] pt-3">
              ...
              <div className="mt-3 border border-[var(--border-color)] rounded-lg overflow-hidden">
                <table className="w-full text-sm">
                  <thead className="bg-[var(--color-base-alt)]">
                    <tr>
                      <th className="px-3 py-2 text-left text-xs uppercase text-secondary font-medium">Broker</th>
                      <th className="px-3 py-2 text-right text-xs uppercase text-secondary font-medium">Cantidad</th>
                      <th className="px-3 py-2 text-right text-xs uppercase text-secondary font-medium">Valor</th>
                      <th className="px-3 py-2 text-right text-xs uppercase text-secondary font-medium">P&L</th>
                    </tr>
                  </thead>
                    ...
                    {investments.map(inv => {
                      ...
                      return (
                        <tr key={inv.id} className="hover:bg-[var(--color-base-alt)]/50">
                          ...
                          <td className="px-3 py-2 text-right font-medium">{fmtQty(inv.quantity)}</td>
                          <td className="px-3 py-2 text-right font-medium">{fmt(currentValue, inv.currency || 'ARS')}</td>
                          <td className={`px-3 py-2 text-right font-medium ${pnl >= 0 ? 'text-success' : 'text-danger'}`}>
                            {pnlPct.toFixed(1)}%
                          </td>
                        </tr>
                      )
            ...
            <div className="mt-4">
              <p className="text-[var(--text-tertiary)] text-xs uppercase mb-2">
                Evolución (vs precio actual {investment.current_price !== null ? fmt(investment.current_price, currency) : '—'})
              </p>
              {loading ? (
                ...
                <div className="border border-[var(--border-color)] rounded-lg p-2 space-y-0.5">
                  <EvolutionRow label="1 día" price={price1d} currentPrice={investment.current_price} currency={currency} />
                  <EvolutionRow label="7 días" price={price7d} currentPrice={investment.current_price} currency={currency} />
                  <EvolutionRow label="30 días" price={price30d} currentPrice={investment.current_price} currency={currency} />
                </div>
              )}
            </div>
          </div>
        ...
        {!isCombined && (
          <div className="border-t border-[var(--border-color)] pt-3">
            <p className="text-[var(--text-tertiary)] text-xs uppercase mb-2">Evolución (vs precio actual {investment.current_price !== null ? fmt(investment.current_price, currency) : '—'})</p>
            {loading ? (
              <p className="text-xs text-[var(--text-tertiary)] py-2">Cargando...</p>
            ) : (
              <div className="border border-[var(--border-color)] rounded-lg p-2 space-y-0.5">
