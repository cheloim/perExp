...
function groupSmallCategories(
  ...
  if (data.length <= maxItems)
    return { shown: data, hidden: [], hiddenTotal: 0 };
...
export default function ExpensesPage() {
  ...
  const { filters, setFilter, clearFilters, searchParams, setSearchParams } =
    useExpenseFilters();
  ...
  const handleBulkApply = () => {
    ...
    if (bulkCategoryId !== "") {
      updateData.category_id =
        bulkCategoryId === "__none__" ? null : parseInt(bulkCategoryId);
    }
  ...
  const pendingDeletes = useRef<Map<number, ReturnType<typeof setTimeout>>>(
    new Map(),
  );
  ...
  const sortedExpenses = useMemo(() => {
    return [...expenses].sort((a, b) => {
      ...
      if (sort.field === "description") {
        const pad = (n: number | null | undefined) =>
  ...
  return (
    <div className="space-y-6 pb-20 md:pb-0">
      ...
      {expenses.length > 0 &&
        (() => {
          const arsExpenses = expenses.filter(
            (e) => (e.currency || "ARS") === "ARS",
          );
          const arsTotal =
            expenseStats?.total ??
            arsExpenses.reduce((s, e) => s + e.amount, 0);
          const totalCount = expenseStats?.count ?? expenses.length;
          const avg =
            expenseStats?.avg ??
            (arsExpenses.length > 0 ? arsTotal / arsExpenses.length : 0);
          const max =
            arsExpenses.length > 0
              ? Math.max(...arsExpenses.map((e) => e.amount))
              : 0;
          return (
            <>
              {/* Stats cards */}
                        ...
                        <ResponsiveContainer width={200} height={200}>
                          <PieChart>
                            ...
                            >
                              {pieData.map((entry, i) => {
                                const isSelected =
                                  selectedDonutCategory === entry.name;
                                const isSearchMatch =
                                  filterSearch &&
                ...
                <th
                  ...
                  onClick={() =>
                    setSort({
                      field: "description",
                      dir:
                        sort.dir === "asc" && sort.field === "description"
                          ? "desc"
                          : "asc",
                    })
                  }
                ...
                <th
                  ...
                  onClick={() =>
                    setSort({
                      field: "category",
                      dir:
                        sort.field === "category" && sort.dir === "asc"
                          ? "desc"
                          : "asc",
                    })
                  }
                ...
                <th
                  ...
                  onClick={() =>
                    setSort({
                      field: "amount",
                      dir:
                        sort.dir === "asc" && sort.field === "amount"
                          ? "desc"
                          : "asc",
                    })
                  }
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            ...
                            <div className="text-xs text-[var(--text-tertiary)] flex gap-1 items-center">
                              ...
                              {exp.person && (
                                <span>{titleCase(exp.person)}</span>
                              )}
                            </div>
                          </td>
                          <td className="px-4 py-3">
                            ...
                            >
                              {exp.category_name ? (
                                ...
                                suggestionsByExpenseId.has(exp.id) ? (
                                <span className="inline-flex items-center gap-1.5">
                                  ...
                                  <button
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      const s = suggestionsByExpenseId.get(
                                        exp.id,
                                      )!;
                                  ...
                                  >
                                    ✨{" "}
                                    {
                                      suggestionsByExpenseId.get(exp.id)!
                                        .category_name
                                    }
                                  </button>
                                </span>
          ...
          <div className="fixed right-0 top-0 z-50 h-full w-full sm:w-80 bg-[var(--color-surface)] border-l border-[var(--border-color)] shadow-gnome-lg flex flex-col">
            <div className="flex items-center justify-between p-4 border-b border-[var(--border-color)]">
              <div>
                ...
                <p className="text-xs text-[var(--text-tertiary)]">
                  {selectedIds.size} gasto{selectedIds.size !== 1 ? "s" : ""}{" "}
                  seleccionado
